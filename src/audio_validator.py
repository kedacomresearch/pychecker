...
class Audio_validator(object):
    def __init__(self, data_dir='test.wav'):
        self._dir = os.path.abspath(data_dir)

        if platform.system() == 'Windows':
            self._dir = os.path.normpath(self._dir).replace('\\', '/')

    def check(self, target_freq=5000, band_with=1000, energy_percent=0.9):
        #打开WAV文档，文件路径根据需要做修改
        ...
        for index in range(0, length):
            ...
            if index > (target_freq - band_with / 2) and index < (
                    target_freq + band_with / 2):
                target_energy += amplitude[index]**2
        percent = target_energy / total_energy
        return percent >= energy_percent
